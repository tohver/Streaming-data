...
In this project, we construct a streaming event pipeline around Apache Kafka and its ecosystem. We use the public data from the [Chicago Transit Authority](https://www.transitchicago.com/data/). This pipeline allows us to simulate and display the status of train lines in real time.

We can monitor a website (like below) to watch trains move from station to station.
...
We configure the train stations to emit some of the events that we need. The CTA has placed a sensor on each side of every train station that can be programmed to take an action whenever a train arrives at the station.

The `value` schema for the arrival event in `producers/models/schemas/arrival_value.json` has following attributes:
	* `station_id`
	* `train_id`
	* `direction`
	* `line`
...
In `producers/models/station.py`:
	* A topic is created for each station in Kafka to track the arrival events
	* The station emits an `arrival` event to Kafka whenever the `Station.run()` function is called.
	* Events emitted to kafka are paired with the Avro `key` and `value` schemas


The `value` schema for the turnstile event is defined in `producers/models/schemas/turnstile_value.json` with the following attributes
	* `station_id`
	* `station_name`
	* `line`

In `producers/models/turnstile.py`:
	* A topic is created for each turnstile for each station in Kafka to track the turnstile events
	* The station emits a `turnstile` event to Kafka whenever the `Turnstile.run()` function is called.
	* Events emitted to kafka are paired with the Avro `key` and `value` schemas

### Step 2: Configuration of Kafka REST Proxy Producer
We send also the weather readings into Kafka from the weather hardware of CTA. Unfortunately, this hardware is old and we cannot use the Python Client Library due to hardware restrictions. Instead, we are going to use HTTP REST to send the data to Kafka from the hardware using Kafka's REST Proxy.

The `value` schema for the weather event is defined in `producers/models/schemas/weather_value.json` with the following attributes
	* `temperature`
	* `status`

In `producers/models/weather.py`:
